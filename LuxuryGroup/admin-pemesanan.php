<?php
    ...
    if(isset($_POST['cari'])){
        ...
        $result = mysqli_query($conn, "SELECT * FROM tb_pemesanan WHERE nama_pemesan LIKE '%$keyword%' OR alamat LIKE '%$keyword%'");
    }else{
...
<body>
    <section class='user-header-index'>
        ...
        <div class='content-index'>
            <h1 class="user-title">DATA PEMESANAN</h1>
            <p class="user-sub-title">Pada pendataan ini admin dapat melihat data diri pemesan dan apa yang telah dipesan mulai dari jenis acara, tanggal, alamat serta note yang ingin ditinggalkan untuk EO . Disini admin dapat mengelola pendataan ini mulai dari menambah, mengubah serta menghapus data klien</p>
        </div>
    </section>
        ...
        <div class="user-table-container">
            ...
            <table class="user-table">
                <tr>
                    ...
                    <th class="user-title-table">No Telpon</th>
                ...
                <tr>
                    ...
                    <td class="user-sub-title-table-kategori"><?php echo $tb_pemesanan['nama_pemesan']; ?></td>
                    <td class="user-sub-title-table-kategori"><?php echo $tb_pemesanan['kategori']; ?></td>
                    <td class="user-sub-title-table-kategori"><?php echo $tb_pemesanan['alamat']; ?></td>
                    <td class="user-sub-title-table"><?php echo $tb_pemesanan['tanggal']; ?></td>
                    <td class="user-sub-title-table"><?php echo $tb_pemesanan['no_telp']; ?></td>
                    <td class="user-sub-title-table-none"><?php echo $tb_pemesanan['note']; ?></td>
                    <td class="user-sub-title-table"> 
                        <a href="admin-delete-pemesanan.php?id=<?php echo $tb_pemesanan["id"]; ?>" onclick="return confirm('Yakin ingin Menghapus pemesanan ini?')" role='button'><i class='bx bxs-trash-alt'></i></a>
                        <a href="admin-update-pemesanan.php?id=<?php echo $tb_pemesanan["id"]; ?>"> <i class='bx bxs-pencil'></i> </a>
                    </td>
                </tr>
                <?php $i++; endforeach; ?>
            </table>
